...
import { calculateAgentCommission, getAgentsApproachingMilestone, getAgentsNeedingUpdate, formatCommissionInfo } from '../utils/commissionCalculator';
...
export const CommissionDashboard: React.FC<CommissionDashboardProps> = ({ employees, site }) => {
  const [selectedTimeframe, setSelectedTimeframe] = useState<'current' | '3months' | '6months' | '1year'>('current');
  ...
  const metrics = useMemo((): CommissionMetrics => {
    const agents = filteredEmployees.filter(emp => emp.role === 'Agent' && emp.status === 'active');
    ...
    const avgMonthsEmployed = agents.length > 0 ? totalMonths / agents.length : 0;
    ...
    const totalEstimatedAnnualCommission = agents.reduce((sum, agent) => {
      ...
      if (calc) {
        ...
        const annualCommission = avgMonthlySales * 12 * (calc.currentCommissionRate);
  ...
  const agentCommissionDetails = useMemo(() => {
    return filteredEmployees
      ...
      .sort((a, b) => (b.commission?.monthsEmployed || 0) - (a.commission?.monthsEmployed || 0));
  }, [filteredEmployees]);
  ...
  const getStatusColor = (agent: Employee) => {
    ...
    if (calc.willChangeToVeteran && calc.daysUntilChange && calc.daysUntilChange <= 7) {
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-gray-600">
            {site ? `${site} Site` : 'All Sites'} • {format(new Date(), 'MMMM yyyy')}
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="bg-white p-6 rounded-lg shadow-sm border">
          ...
          <div className="mt-2 flex text-xs">
            ...
            <span className="text-green-600">{metrics.veteranAgents} veteran</span>
          </div>
        </div>
        ...
        <div className="bg-white p-6 rounded-lg shadow-sm border">
          <div className="flex items-center">
            ...
            <div className="ml-3">
              ...
              <p className="text-2xl font-bold text-gray-900">
                {metrics.agentsApproachingMilestone.length + metrics.agentsNeedingUpdate.length}
              </p>
            </div>
          </div>
          <div className="mt-2 text-xs">
            <div className="text-amber-600">{metrics.agentsApproachingMilestone.length} approaching milestone</div>
            <div className="text-red-600">{metrics.agentsNeedingUpdate.length} need updates</div>
          </div>
        </div>
        ...
        <div className="bg-white p-6 rounded-lg shadow-sm border">
          <div className="flex items-center">
            ...
            <div className="ml-3">
              <p className="text-sm font-medium text-gray-600">Est. Annual Commission</p>
              <p className="text-2xl font-bold text-gray-900">
                ${Math.round(metrics.totalEstimatedAnnualCommission / 1000)}k
              </p>
            </div>
          </div>
          <p className="text-xs text-gray-500 mt-2">Based on avg $5k monthly sales</p>
        </div>
      </div>

      {/* Alerts Section */}
      {(metrics.agentsApproachingMilestone.length > 0 || metrics.agentsNeedingUpdate.length > 0) && (
        <div className="bg-amber-50 border border-amber-200 rounded-lg p-4">
          ...
          <div className="space-y-3">
            {metrics.agentsApproachingMilestone.length > 0 && (
              <div>
                <h4 className="font-medium text-amber-800 mb-2">Approaching 6-Month Milestone:</h4>
                <div className="space-y-1">
                  {metrics.agentsApproachingMilestone.map(agent => {
                    const calc = calculateAgentCommission(agent);
                    return (
                      <div key={agent.id} className="text-sm text-amber-700">
                        <strong>{agent.name}</strong> - {calc?.daysUntilChange} days until veteran status
                      </div>
                    );
                  })}
                </div>
            ...
            {metrics.agentsNeedingUpdate.length > 0 && (
              <div>
                <h4 className="font-medium text-red-800 mb-2">Need Commission Tier Update:</h4>
                <div className="space-y-1">
                  {metrics.agentsNeedingUpdate.map(agent => {
                    const calc = calculateAgentCommission(agent);
                    return (
                      <div key={agent.id} className="text-sm text-red-700">
                        <strong>{agent.name}</strong> - {calc?.monthsEmployed} months employed, still marked as 'new'
                      </div>
                    );
                  })}
                </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody className="bg-white">
                {agentCommissionDetails.map(({ employee, commission }, index) => {
                  const StatusIcon = getStatusIcon(employee);
                  return (
                    <tr key={employee.id} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                      <td className="px-6 py-4 text-sm font-medium text-gray-900">
                        {employee.name}
                        <div className="text-xs text-gray-500">{employee.site}</div>
                      </td>
                      <td className="px-6 py-4 text-sm text-gray-900">
                        {commission?.monthsEmployed || 0} months
                        <div className="text-xs text-gray-500">
                          Since {format(new Date(employee.startDate), 'MMM yyyy')}
                        </div>
                      </td>
                      <td className="px-6 py-4 text-sm">
                        <span className={`inline-block px-2 py-1 text-xs font-medium rounded-full ${
                          commission?.tier === 'veteran' ? 'bg-green-100 text-green-800' : 'bg-blue-100 text-blue-800'
                        }`}>
                          {commission?.tier === 'veteran' ? 'Veteran' : 'New'}
                        </span>
                      </td>
                      <td className="px-6 py-4 text-sm text-gray-900">
                        {commission ? `${(commission.currentCommissionRate * 100).toFixed(0)}%` : 'N/A'}
                        <div className="text-xs text-gray-500">
                          ${commission?.currentSalary.toLocaleString() || 'N/A'} base
                        </div>
                      </td>
                      <td className="px-6 py-4 text-sm">
                        <div className={`flex items-center ${getStatusColor(employee)}`}>
                          <StatusIcon className="w-4 h-4 mr-1" />
                          {commission?.willChangeToVeteran && commission.daysUntilChange ? (
                            <span>{commission.daysUntilChange} days to veteran</span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white p-4 rounded-lg shadow-sm border">
          <h4 className="font-medium text-gray-900 mb-2">Commission Distribution</h4>
        ...
        <div className="bg-white p-4 rounded-lg shadow-sm border">
          ...
          <div className="space-y-2 text-sm">
            <div className="flex justify-between">
              ...
              <span className="font-medium text-amber-600">{metrics.agentsApproachingMilestone.length}</span>
            </div>
            <div className="flex justify-between">
              <span>Need updates</span>
              <span className="font-medium text-red-600">{metrics.agentsNeedingUpdate.length}</span>
            </div>
          </div>
        </div>

        <div className="bg-white p-4 rounded-lg shadow-sm border">
          <h4 className="font-medium text-gray-900 mb-2">Compensation Overview</h4>
          <div className="space-y-2 text-sm">
            <div className="flex justify-between">
              <span>Avg Base Salary</span>
              <span className="font-medium">
                ${Math.round((metrics.newAgents * 60000 + metrics.veteranAgents * 30000) / Math.max(metrics.totalAgents, 1) / 1000)}k
              </span>
            </div>
            <div className="flex justify-between">
              <span>Total Est. Commission</span>
              <span className="font-medium text-green-600">
                ${Math.round(metrics.totalEstimatedAnnualCommission / 1000)}k/year
              </span>
            </div>
          </div>
        </div>
