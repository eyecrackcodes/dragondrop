...
console.log(
  ...
  process.env.REACT_APP_FIREBASE_API_KEY
    ? `[SET: ${process.env.REACT_APP_FIREBASE_API_KEY.substring(0, 10)}...]`
    : "[MISSING]"
);
console.log(
  "🔧 REACT_APP_FIREBASE_DATABASE_URL:",
  process.env.REACT_APP_FIREBASE_DATABASE_URL || "[MISSING]"
);
console.log(
  "🔧 REACT_APP_FIREBASE_PROJECT_ID:",
  process.env.REACT_APP_FIREBASE_PROJECT_ID || "[MISSING]"
);
console.log(
  "🔧 All Firebase env vars found:",
  Object.keys(process.env).filter((key) =>
    key.startsWith("REACT_APP_FIREBASE_")
  )
);
...
const firebaseConfig = {
  apiKey:
    process.env.REACT_APP_FIREBASE_API_KEY ||
    "AIzaSyA-4im1CI-Ds3f2unSkNUViBEcqIkI7jAE",
  authDomain:
    process.env.REACT_APP_FIREBASE_AUTH_DOMAIN ||
    "dragondrop-9802e.firebaseapp.com",
  databaseURL:
    process.env.REACT_APP_FIREBASE_DATABASE_URL ||
    "https://dragondrop-9802e-default-rtdb.firebaseio.com",
  projectId: process.env.REACT_APP_FIREBASE_PROJECT_ID || "dragondrop-9802e",
  storageBucket:
    process.env.REACT_APP_FIREBASE_STORAGE_BUCKET ||
    "dragondrop-9802e.firebasestorage.app",
  messagingSenderId:
    process.env.REACT_APP_FIREBASE_MESSAGING_SENDER_ID || "973998104962",
  appId:
    process.env.REACT_APP_FIREBASE_APP_ID ||
    "1:973998104962:web:5733e8dc8539e4771d2f9b",
  measurementId:
    process.env.REACT_APP_FIREBASE_MEASUREMENT_ID || "G-W1QKTP0NM2",
};
...
const validateFirebaseConfig = () => {
  ...
  if (missing.length > 0) {
    ...
    console.warn(
      "💡 App will run in demo mode. Check .env file with Firebase credentials."
    );
    return false;
  }
...
export const EmployeeService = {
  ...
  // Terminate employee with comprehensive details
  async terminate(employeeId: string, terminationDetails: any): Promise<void> {
    try {
  ...
  // Legacy terminate method for backward compatibility
  async terminateBasic(employeeId: string): Promise<void> {
    try {
  ...
  // Bulk operations
  async bulkUpdate(
    updates: Array<{ id: string; data: Partial<Employee> }>
  ): Promise<void> {
    try {
  ...
  onEmployeesChange(callback: (employees: Employee[]) => void): () => void {
    if (!database) {
      console.warn(
        "Firebase not configured - returning empty unsubscribe function"
      );
      return () => {};
    }
...
export const exitSurveyService = {
  ...
  getByEmployee: async (employeeId: string) => {
    ...
    return Object.values(surveys).filter(
      (survey: any) => survey.employeeId === employeeId
    );
  },
};
...
export const changeLogService = {
  ...
  getByEmployee: async (employeeId: string) => {
    ...
    return Object.values(changes).filter(
      (change: any) => change.employeeId === employeeId
    );
  },
...
export const FirebaseUtils = {
  ...
  getConfigStatus(): {
    ...
    const missing = requiredFields.filter((field) => {
      const value = firebaseConfig[field as keyof typeof firebaseConfig];
      return (
        !value || value === "" || value === `your_${field.toLowerCase()}_here`
      );
    });
