...
import { calculateAgentCommission, getAgentsApproachingMilestone, checkCommissionEligibility } from '../utils/commissionCalculator';
...
export const useFirebaseEmployees = () => {
    ...
    const initializeEmployees = async () => {
      try {
        ...
        try {
          ...
          console.warn('Firebase connection test failed, but will try to continue');
  ...
  const moveEmployee = useCallback(async (employeeId: string, newManagerId: string) => {
    try {
      setError(null);
      await EmployeeService.moveToManager(employeeId, newManagerId);
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'Failed to move employee';
      setError(errorMessage);
      throw new Error(errorMessage);
    }
  }, []);

  const transferEmployee = useCallback(async (employeeId: string, newSite: Site) => {
    try {
      setError(null);
      await EmployeeService.transferToSite(employeeId, newSite);
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'Failed to transfer employee';
      setError(errorMessage);
      throw new Error(errorMessage);
    }
  }, []);

  const promoteEmployee = useCallback(async (employeeId: string, newRole: Role) => {
    try {
      setError(null);
      await EmployeeService.promote(employeeId, newRole);
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'Failed to promote employee';
      setError(errorMessage);
      throw new Error(errorMessage);
    }
  }, []);

  const terminateEmployee = useCallback(async (employeeId: string) => {
    try {
      setError(null);
      await EmployeeService.terminate(employeeId);
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'Failed to terminate employee';
      setError(errorMessage);
      throw new Error(errorMessage);
    }
  }, []);

  const updateCommissionTier = useCallback(async (employeeId: string, tier: CommissionTier) => {
    try {
      setError(null);
      await EmployeeService.updateCommissionTier(employeeId, tier);
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'Failed to update commission tier';
      setError(errorMessage);
      throw new Error(errorMessage);
    }
  }, []);

  const bulkUpdateEmployees = useCallback(async (updates: Array<{ id: string; data: Partial<Employee> }>) => {
    try {
      setError(null);
  ...
  return {
    ...
    refetch: () => {
      ...
      console.log('Data is automatically synchronized via Firebase real-time listeners');
    }
  };
};
  ...
  useEffect(() => {
    ...
    const initializeSiteEmployees = async () => {
      try {
        ...
        unsubscribe = EmployeeService.onSiteEmployeesChange(site, (employeeData) => {
          setEmployees(employeeData);
          setIsLoading(false);
        });
...
export const useFirebaseOrgStructure = (site?: Site) => {
  ...
  const { employees, isLoading, error } = site ? siteEmployeesData : allEmployeesData;

  const orgStructure = employees.reduce((acc, employee) => {
  ...
  const orgTree = orgStructure.directors.map(director => ({
    ...director,
    reports: orgStructure.managers.filter(manager => manager.managerId === director.id).map(manager => ({
      ...manager,
      reports: orgStructure.teamLeads.filter(lead => lead.managerId === manager.id).map(lead => ({
        ...lead,
        reports: orgStructure.agents.filter(agent => agent.managerId === lead.id)
      }))
    }))
  }));
...
export const useFirebaseCommissionAlerts = () => {
  ...
  const hasAlerts = agentsApproachingMilestone.length > 0 || agentsNeedingUpdate.length > 0;
    ...
    mutate: async (params: { 
      ...
      try {
        ...
        if (params.newManagerId) {
          await EmployeeService.moveToManager(params.employeeId, params.newManagerId);
        }
        
        if (params.newSite) {
          await EmployeeService.transferToSite(params.employeeId, params.newSite);
        }
        
        if (params.status) {
          if (params.status === 'terminated') {
            await EmployeeService.terminate(params.employeeId);
          } else {
            await EmployeeService.update(params.employeeId, { status: params.status });
          }
        }
